...
import com.yhy.carrot.engine.ImageEngine;
...
public final class SelectionCreator {
     ...
     * @param mediaTypeSet MIME type set to select.
     */
    public SelectionCreator(Carrot matisse, @NonNull Set<MediaType> mediaTypeSet, boolean mediaTypeExclusive) {
        mCarrot = matisse;
        mSelectionSpec = SelectionSpec.getCleanInstance();
        mSelectionSpec.mediaTypeSet = mediaTypeSet;
     ...
     * <p>
     * It's a redundant API with {@link Carrot#obtainResult(Intent)},
    ...
    public void forResult(int requestCode) {
        Activity activity = mCarrot.getActivity();
        if (activity == null) {
            return;
        }

        Intent intent = new Intent(activity, CarrotActivity.class);

        Fragment fragment = mCarrot.getFragment();
        if (fragment != null) {
