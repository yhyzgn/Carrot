...
public class GifSizeFilter extends Filter {
    ...
    @Override
    public Set<MediaType> constraintTypes() {
    ...
    public IncapableCause filter(Context context, Item item) {
        ...
        Point size = PhotoUtils.getBitmapBound(context.getContentResolver(), item.getContentUri());
        if (size.x < mMinWidth || size.y < mMinHeight || item.size > mMaxSize) {
            return new IncapableCause(IncapableCause.DIALOG, context.getString(R.string.error_gif, mMinWidth,
                    String.valueOf(PhotoUtils.getSizeInMB(mMaxSize))));
        }
        return null;
    }
